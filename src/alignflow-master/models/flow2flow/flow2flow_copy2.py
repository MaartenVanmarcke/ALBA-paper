...
class Flow2Flow(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.g_src = [RealNVP(
                             un_normalize_x=True,
                             no_latent=False) for i in range(self.num_sources)]
        for model in self.g_src:
            util.init_model(model, init_method=args.initializer)

        if self.is_training:
            # Set up discriminators
            self.d_tgt = [PatchGAN(args) for _ in range(self.num_sources)]  # Answers Q "is this tgt image real?"
            ...
            self.src2tgt_buffer = [[0]*self.num_sources for _ in range(self.num_sources)]
            self.src2tgt_buffer[0][1] = util.ImageBuffer(buffer_capacity)  # Buffer of generated tgt images
            self.src2tgt_buffer[1][0] = util.ImageBuffer(buffer_capacity)  # Buffer of generated src images
        else:
            self._data_parallel()
    ...
    def test(self):
        ...
        with torch.no_grad():
            src2lat, _ = self.g_src[0](self.src[0], reverse=False)
            src2lat2tgt, _ = self.g_src[1](src2lat, reverse=True)
            self.src2tgt = torch.tanh(src2lat2tgt)

            tgt2lat, _ = self.g_src[1](self.src[1], reverse=False)
            tgt2lat2src, _ = self.g_src[0](tgt2lat, reverse=True)
            self.tgt2src = torch.tanh(tgt2lat2src)
        return [src2lat, tgt2lat], [[0,src2lat2tgt],[ tgt2lat2src,0]]
    ...
    def backward_d(self):
        ...
        src2tgt = self.src2tgt_buffer[0][1].sample(self.src2tgt)
        self.loss_d_tgt = self._forward_d(self.d_tgt[1], self.src[1], src2tgt)

        # Forward src discriminator
        tgt2src = self.src2tgt_buffer[1][0].sample(self.tgt2src)
        self.loss_d_src = self._forward_d(self.d_tgt[0], self.src[0], tgt2src)
    ...
    def backward_g(self):
        ...
        self.src2lat[0], sldj_src2lat = self.g_src[0](self.src[0], reverse=False)
        self.loss_mle_src = self.lambda_mle * self.mle_loss_fn(self.src2lat[0], sldj_src2lat)

        # Finish src -> lat -> tgt: Say target is real to invert loss
        src2tgt, _ = self.g_src[1](self.src2lat[0], reverse=True)
        self.src2tgt = torch.tanh(src2tgt)

        # Forward tgt -> lat: Get MLE loss
        self.src2lat[1], sldj_tgt2lat = self.g_src[1](self.src[1], reverse=False)
        self.loss_mle_tgt = self.lambda_mle * self.mle_loss_fn(self.src2lat[1], sldj_tgt2lat)

        # Finish tgt -> lat -> src: Say source is real to invert loss
        tgt2src, _ = self.g_src[0](self.src2lat[1], reverse=True)
        ...
        if self.clamp_jacobian:
            ...
            self.loss_jc_src = self.jc_loss_fn(self.src2tgt, self.src2tgt_jc, self.src[0], self.src_jc)
            self.loss_jc_tgt = self.jc_loss_fn(self.tgt2src, self.tgt2src_jc, self.src[1], self.tgt_jc)
            self.loss_jc = self.loss_jc_src + self.loss_jc_tgt
        else:
            self.loss_jc_src = self.loss_jc_tgt = self.loss_jc = 0.

        # GAN loss
        self.loss_gan_src = self.gan_loss_fn(self.d_tgt[1](self.src2tgt), is_tgt_real=True)
        self.loss_gan_tgt = self.gan_loss_fn(self.d_tgt[0](self.tgt2src), is_tgt_real=True)
    ...
    def get_image_dict(self):
        ...
        image_tensor_dict = {'src': self.src[0],
    ...
    def _data_parallel(self):
        self.g_src[0] = nn.DataParallel(self.g_src[0], self.gpu_ids).to(self.device)
        self.g_src[1] = nn.DataParallel(self.g_src[1], self.gpu_ids).to(self.device)
        if self.is_training:
            self.d_tgt[0] = nn.DataParallel(self.d_tgt[0], self.gpu_ids).to(self.device)
            self.d_tgt[1] = nn.DataParallel(self.d_tgt[1], self.gpu_ids).to(self.device)
    ...
    def _jc_preprocess(self):
        ...
        """
        delta = torch.randn_like(self.src[0])
        src_jc = self.src[0] + delta / delta.norm()
        src_jc.clamp_(-1, 1)
        self.src[0] = torch.cat((self.src[0], src_jc), dim=0)

        delta = torch.randn_like(self.src[1])
        tgt_jc = self.src[1] + delta / delta.norm()
        tgt_jc.clamp_(-1, 1)
        self.src[1] = torch.cat((self.src[1], tgt_jc), dim=0)
    ...
    def _jc_postprocess(self):
        ...
        See Also:
            Algorithm 1 from https://arxiv.org/1802.08768v2
        """
        self.src[0], self.src_jc = self.src[0].chunk(2, dim=0)
        self.src[1], self.tgt_jc = self.src[1].chunk(2, dim=0)
