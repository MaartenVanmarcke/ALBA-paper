...
class PlotPerformance:
    ...
    def __call__(self, *args: Any, **kwds: Any) -> Any:
            ...
            dummy = np.sum(namesMethods["SmartInitialGuess"]== np.argmax(aucroc[:,:,namesDatasets["40_Vowels0"],:], axis = 0))
            dummy1 = dummy
            print("Overall", dummy, np.prod(aucroc[namesMethods["SmartInitialGuess"],:,namesDatasets["40_Vowels0"],:].shape),dummy/ np.prod(aucroc[namesMethods["SmartInitialGuess"],:,namesDatasets["40_Vowels0"],:].shape))
            dummy = np.sum(namesMethods["BasicActiveLearning"]== np.argmax(aucroc[:,:,:,:], axis = 0))
            dummy2=dummy
            print("Overall Uncertainty", dummy, np.prod(aucroc[namesMethods["BasicActiveLearning"],:,:,:].shape),dummy/ np.prod(aucroc[namesMethods["BasicActiveLearning"],:,:,:].shape))
            dummy = np.sum(namesMethods["AlbaMethod"]== np.argmax(aucroc[:,:,:,:], axis = 0))
            dummy3=dummy
            print("Overall Random", dummy, np.prod(aucroc[namesMethods["RandomSampling"],:,:,:].shape),dummy/ np.prod(aucroc[namesMethods["RandomSampling"],:,:,:].shape))
            dummy = np.sum(namesMethods["AlbaMethod"]== np.argmax(aucroc[:,:,:,:], axis = 0))
            dummy4=dummy
            print("Overall ALBA", dummy, np.prod(aucroc[namesMethods["AlbaMethod"],:,:,:].shape),dummy/ np.prod(aucroc[namesMethods["AlbaMethod"],:,:,:].shape))
