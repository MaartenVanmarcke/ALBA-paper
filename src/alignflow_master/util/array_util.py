...
def checkerboard_like(x, reverse=False):
    ...
    """
    number_of_examples, number_of_features = x.shape[0], x.shape[1]
    checkerboard = np.zeros((number_of_examples, math.ceil(number_of_features/2)))
    checkerboard = np.hstack((checkerboard,np.ones((number_of_examples, number_of_features-math.ceil(number_of_features/2)))))
    if checkerboard.shape != x.shape:
